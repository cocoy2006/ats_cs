...
public class ApplicationDao extends BaseDao<Application> {
	...
	public Application findByRunnerId(int runnerId) {
		String hql = "FROM Application WHERE id=(SELECT applicationId FROM Testcase WHERE id=(SELECT testcaseId FROM CsDispatcher WHERE id=(SELECT dispatcherId FROM CsRunner WHERE id=?)))";
